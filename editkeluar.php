...
$query  = "SELECT * FROM keluar WHERE idkeluar = $id";
...
if (isset($_POST['update'])) {
    ...
    $query = "UPDATE keluar SET
        ...
        tglsuratkeluar = '$tglsurat',
        tujuan = '$asal',
        perihalkeluar = '$perihal',
        orangkeluar = '$orang'
        WHERE idkeluar = $id";
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mt-4 mb-4 text-center">
            <h1 class="h3 mt-4 mb-4 text-gray-800">Edit Surat Keluar</h1>
          </div>

          <div class="card shadow mb-4">
            <div class="card-body">
              <form action="editkeluar.php?id=<?= htmlspecialchars($row['idkeluar']); ?>" method="POST">
                <input type="hidden" name="id" value="<?= htmlspecialchars($row['idkeluar']); ?>">

                <div class="form-group">
                  <label for="nosurat">Nomor Surat</label>
                  <input type="text" class="form-control" id="nosurat" name="nosurat" value="<?= htmlspecialchars($row['nosuratkeluar']); ?>" required>
                </div>

                <div class="form-group">
                  <label for="tglsurat">Tanggal Surat</label>
                  <input type="date" class="form-control" id="tglsurat" name="tglsurat" value="<?= htmlspecialchars($row['tglsuratkeluar']); ?>" required>
                </div>

                <div class="form-group">
                  <label for="asal">Tujuan</label>
                  <input type="text" class="form-control" id="asal" name="asal" value="<?= htmlspecialchars($row['tujuan']); ?>" required>
                </div>

                <div class="form-group">
                  <label for="perihal">Perihal</label>
                  <input type="text" class="form-control" id="perihal" name="perihal" value="<?= htmlspecialchars($row['perihalkeluar']); ?>" required>
                </div>

                <div class="form-group">
                  <label for="orang">Pembuat</label>
                  <input type="text" class="form-control" id="orang" name="orang" value="<?= htmlspecialchars($row['orangkeluar']); ?>" required>
                </div>
